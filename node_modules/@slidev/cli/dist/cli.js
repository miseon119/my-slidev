...
var _chunkVIIDVLUIjs = require('./chunk-VIIDVLUI.js');
...
var _chunkIMHRVD6Ijs = require('./chunk-IMHRVD6I.js');
...
var import_fast_deep_equal = _chunkQT3EQ43Ajs.__toModule.call(void 0, _chunk2IUA7VOFjs.require_fast_deep_equal.call(void 0, ));
...
var cli = _yargs2.default.scriptName("slidev").usage("$0 [args]").version(_chunkIMHRVD6Ijs.version).strict().showHelpOnFail(false).alias("h", "help").alias("v", "version");
cli.command("* [entry]", "Start a local server for Slidev", (args) => commonOptions(args).option("port", {
  ...
  async function initServer() {
    if (server)
      await server.close();
    const options = await _chunkIMHRVD6Ijs.resolveOptions.call(void 0, { entry, theme }, "dev");
    port = userPort || await findFreePort(3030);
    server = await _chunkVIIDVLUIjs.createServer.call(void 0, options, {
      server: {
        port,
        strictPort: true,
        open,
      ...
      onDataReload(newData, data) {
        if (!theme && _chunkIMHRVD6Ijs.resolveThemeName.call(void 0, newData.config.theme) !== _chunkIMHRVD6Ijs.resolveThemeName.call(void 0, data.config.theme)) {
...
cli.command("build [entry]", "Build hostable SPA", (args) => commonOptions(args).option("watch", {
  ...
  const { build } = await Promise.resolve().then(() => _chunkQT3EQ43Ajs.__toModule.call(void 0, _chunkQT3EQ43Ajs.__require.call(void 0, "./build-DBBNRLZC.js")));
  const options = await _chunkIMHRVD6Ijs.resolveOptions.call(void 0, { entry, theme }, "build");
...
cli.command("format [entry]", "Format the markdown file", (args) => commonOptions(args).strict().help(), async ({ entry }) => {
  const data = await _chunkIMHRVD6Ijs.fs_exports.load(entry);
  _chunkIMHRVD6Ijs.fs_exports.prettify(data);
  await _chunkIMHRVD6Ijs.fs_exports.save(data);
});
cli.command("theme [subcommand]", "Theme related operations", (command) => {
  return command.command("eject", "Eject current theme into local file system", (args) => commonOptions(args).option("dir", {
    ...
    const data = await _chunkIMHRVD6Ijs.fs_exports.load(entry);
    const theme = _chunkIMHRVD6Ijs.resolveThemeName.call(void 0, themeInput || data.config.theme);
    if (theme === "none") {
      console.error('Can not eject theme "none"');
      process.exit(1);
    }
    if (_chunkIMHRVD6Ijs.isPath.call(void 0, theme)) {
      console.error("Theme is already ejected");
      process.exit(1);
    }
    const roots = _chunkIMHRVD6Ijs.getThemeRoots.call(void 0, theme, entry);
    if (!roots.length) {
...
cli.command("export [entry]", "Export slides to PDF", (args) => commonOptions(args).option("output", {
  ...
  const { exportSlides } = await Promise.resolve().then(() => _chunkQT3EQ43Ajs.__toModule.call(void 0, _chunkQT3EQ43Ajs.__require.call(void 0, "./export-K6LZ37II.js")));
  const port = await findFreePort(12445);
  const options = await _chunkIMHRVD6Ijs.resolveOptions.call(void 0, { entry, theme }, "build");
  const server = await _chunkVIIDVLUIjs.createServer.call(void 0, options, {
    server: { port },
    clearScreen: false
  });
  await server.listen(port);
  printInfo(options);
  _chunkIMHRVD6Ijs.fs_exports.filterDisabled(options.data);
  const width = 1920;
...
function printInfo(options, port, remote) {
  ...
  console.log(`${_kolorist.bold.call(void 0, "  Slidev")}  ${_kolorist.blue.call(void 0, `v${_chunkIMHRVD6Ijs.version}`)} ${_isinstalledglobally2.default ? _kolorist.yellow.call(void 0, "(global)") : ""}`);
  console.log();
